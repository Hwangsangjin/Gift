...
IndexBufferPtr Graphics::CreateIndexBuffer(void* index_list, UINT index_count)
{
    return std::make_shared<IndexBuffer>(index_list, index_count, this);
}

VertexBufferPtr Graphics::CreateVertexBuffer(void* vertex_list, UINT vertex_size, UINT vertex_count)
{
    return std::make_shared<VertexBuffer>(vertex_list, vertex_size, vertex_count, this);
}

VertexShaderPtr Graphics::CreateVertexShader(const wchar_t* file_path)
{
    return std::make_shared<VertexShader>(file_path, this);
}

PixelShaderPtr Graphics::CreatePixelShader(const wchar_t* file_path)
{
    return std::make_shared<PixelShader>(file_path, this);
}

Texture2DPtr Graphics::CreateTexture(const wchar_t* file_path)
{
    return std::make_shared<Texture2D>(file_path, this);
}

Texture2DPtr Graphics::CreateTexture(const Rect& size, Texture2D::Type type)
{
...
const void* Graphics::GetMeshLayoutByteCode() const
{
    return m_mesh_layout_byte_code;
}

size_t Graphics::GetMeshLayoutSize() const
{
...
void Graphics::CompilePrivateShader()
{
