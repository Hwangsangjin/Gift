...
Texture2D::Texture2D(const wchar_t* file_path, Graphics* graphics)
	: m_graphics(graphics)
{	
	// 텍스처 생성
	DirectX::ScratchImage image_data;
	DirectX::LoadFromWICFile(file_path, DirectX::WIC_FLAGS_IGNORE_SRGB, nullptr, image_data);
	DirectX::CreateTexture(m_graphics->GetD3DDevice(), image_data.GetImages(), image_data.GetImageCount(), image_data.GetMetadata(), &m_texture);
	assert(m_texture);
	...
	// 셰이더 리소스 뷰 생성
	m_graphics->GetD3DDevice()->CreateShaderResourceView(m_texture.Get(), &desc, &m_shader_resource_view);
	...
	// 샘플러 스테이트 생성
	m_graphics->GetD3DDevice()->CreateSamplerState(&sampler_desc, &m_sampler_state);
	assert(m_sampler_state);
}

Texture2D::Texture2D(const Rect& size, Texture2D::Type type, Graphics* graphics)
	: m_graphics(graphics)
{
	// 텍스처 타입에 따라서 바인딩
	...
	m_graphics->GetD3DDevice()->CreateTexture2D(&texture_desc, nullptr, (ID3D11Texture2D**)m_texture.GetAddressOf());
	assert(m_texture);

	if (type == Texture2D::Type::RenderTarget)
	{
		m_graphics->GetD3DDevice()->CreateShaderResourceView(m_texture.Get(), nullptr, &m_shader_resource_view);
		assert(m_shader_resource_view);

		m_graphics->GetD3DDevice()->CreateRenderTargetView(m_texture.Get(), nullptr, &m_render_target_view);
		assert(m_render_target_view);
	}
	else if (type == Texture2D::Type::DepthStencil)
	{
		m_graphics->GetD3DDevice()->CreateDepthStencilView(m_texture.Get(), nullptr, &m_depth_stencil_view);
